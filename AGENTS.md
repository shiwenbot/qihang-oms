...
发版形态是 **Windows 免安装包**：内置 MySQL/Redis/JRE/Python，用户解压即用（详见 `package/windows/README.txt`）。
...
| `package/windows/` | 发版脚本：启动/停止/打包/首装自测/快照净化 |
...
| `Build-Package.ps1` | 一键打包：npm build → 嵌入 jar → mvn → 组装 → BUILD-INFO(SHA256) → 首装门禁 → zip |
...
→ package\out\QihangOMS-<时间>-<git短哈希>.zip → 用户覆盖解压重启
```
