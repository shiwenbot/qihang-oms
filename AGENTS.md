...
发版形态是 **一个 Windows Setup.exe**：内置 MySQL/Redis/JRE/Python；双击安装到纯英文路径，打开后在窗口内使用系统（详见 `package/windows/README.txt`）。
...
| `Build-Package.ps1` | 一键打包：npm build → 嵌入 jar → mvn → 组装 → QihangOMS.exe → BUILD-INFO(SHA256) → 首装门禁 → Setup.exe |
...
用户：双击 Setup.exe 安装/覆盖升级（不删 runtime\mysql\data），打开“启航电商 OMS”
```
